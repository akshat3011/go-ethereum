...
func TestIterator(t *testing.T) {
	...
	vals := []struct{ k, v, d string }{
		...
		{"shaman", "horse","ssid"},
		{"doge", "coin","labra"},
		{"dog", "puppy","wildest dream"},
		{"somethingveryoddindeedthis is", "myothernodedata","lana del rey"},
	}
...
func TestIteratorSeek(t *testing.T) {
	...
	for _, val := range testdata1 {
		trie.Update([]byte(val.k), []byte(val.v),[]byte("cat"))
	}
...
func TestIteratorNoDups(t *testing.T) {
	var tr Trie
	for _, val := range testdata1 {
		tr.Update([]byte(val.k), []byte(val.v),[]byte("cat13"))
	}
...
func testIteratorContinueAfterSeekError(t *testing.T, memonly bool) {
	...
	for _, val := range testdata1 {
		ctr.Update([]byte(val.k), []byte(val.v),[]byte("ddd"))
	}
