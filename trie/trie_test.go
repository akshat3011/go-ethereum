...
func TestNull(t *testing.T) {
	...
	key := make([]byte, 32)
	value := []byte("test")
	trie.Update(key, value,[]byte("testdata"))
	v,_:=trie.Get(key)
...
func testMissingNode(t *testing.T, memonly bool) {
	...
	updateString(trie, "120000", "ariana","intoyou")
	updateString(trie, "123456", "taylor","swift")
	...
	//fmt.Println("yuy")
	_,_,err := trie.TryGet([]byte("120000"))
	
	if err != nil {
		t.Errorf("Unexpected error: %v", err)
	}
	trie, _ = New(root, triedb)
	_,_,err = trie.TryGet([]byte("120099"))
	if err != nil {
		t.Errorf("Unexpected error: %v", err)
	}
	trie, _ = New(root, triedb)
	_,_,err = trie.TryGet([]byte("123456"))
	if err != nil {
		t.Errorf("Unexpected error: %v", err)
	}
	trie, _ = New(root, triedb)
	err = trie.TryUpdate([]byte("120099"), []byte("zxcvzxcvzxcvzxcvzxcvzxcvzxcvzxcv"),[]byte("baby"))
	if err != nil {
		t.Errorf("Unexpected error: %v", err)
	}
	trie, _ = New(root, triedb)
	...
	trie, _ = New(root, triedb)
	err = trie.TryUpdate([]byte("120099"), []byte("zxcv"), []byte("sad"))
	if _, ok := err.(*MissingNodeError); !ok {
		t.Errorf("Wrong error: %v", err)
	}
	trie, _ = New(root, triedb)
...
func TestInsert(t *testing.T) {
	...
	trie = newEmpty()
	updateString(trie, "A", "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","move")
...
func TestGet(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		res,_ := getString(trie, "dog")
		if !bytes.Equal(res, []byte("puppy")) {
			t.Errorf("expected puppy got %x", res)
		}

		unknown,_ := getString(trie, "unknown")
		if unknown != nil {
...
func TestLargeValue(t *testing.T) {
	trie := newEmpty()
	trie.Update([]byte("key1"), []byte{99, 99, 99, 99},[]byte("testdata"))
	trie.Update([]byte("key2"), bytes.Repeat([]byte{1}, 32),[]byte("testdata"))
	trie.Hash()
}
...
func TestCacheUnload(t *testing.T) {
	...
	updateString(trie, key1, "this is the branch of key1.","testdata")
	updateString(trie, key2, "this is the branch of key2.","testdata")
...
func runRandTest(rt randTest) bool {
	...
	for i, step := range rt {
		switch step.op {
		case opUpdate:
			tr.Update(step.key, step.value,[]byte("testdata"))
		...
		case opItercheckhash:
			...
			for it.Next() {
				checktr.Update(it.Key, it.Value,[]byte("testdata"))
			}
...
func BenchmarkHash(b *testing.B) {
	...
	trie := newEmpty()
	for i := 0; i < len(addresses); i++ {
		trie.Update(crypto.Keccak256(addresses[i][:]), accounts[i],[]byte("testdata"))
	}
	b.ResetTimer()
	b.ReportAllocs()
	trie.Hash()
